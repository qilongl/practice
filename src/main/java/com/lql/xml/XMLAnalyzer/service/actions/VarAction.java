...
import com.lql.xml.XMLAnalyzer.util.*;
...
public class VarAction implements Serializable {
    ...
    public VarAction(BusiConfig busiConfig) {
        ...
        try {
            ...
            for (int u = 0; u < range; u++) {
                ...
                if (ParameterUtil.hasChildrenNamed(rootElement, AttrIterator)) {
                    // 取到入参数据
                    List<Map<String, Object>> paramsArray = TypeUtil.changeToListMap(busiConfig.getParamsFromInParams_CmdResult(paramName, inParams));
                    // 取到参数模型
                    Parameters paramModel = busiConfig.getParametersMap().get(paramName);
                    // 取到参数列表
                    ...
                    for (int a = 0; a < paramsArray.size(); a++) {
                        ...
                        while (it.hasNext()) {
                            ...
                            for (int i = 0; i < elementList.size(); i++) {
                                ...
                                Object rsObject = ParameterUtil.getValue(busiConfig, paramName, eleValue, inParams, jsonObject);
                                if (null == rsObject)
                    ...
                    if (!StringHelper.isNotNull(paramName)) {
                        ...
                        for (int j = 0; j < elementList.size(); j++) {
                            ...
                            if (ExpressionUtil.machExpression(patten1, elementValue) || ExpressionUtil.machExpression(patten2, elementValue)) {
                                Object rsObject = ParameterUtil.getValue(busiConfig, paramName, elementValue, inParams, null);
                        ...
                        for (int i = 0; i < paramsArray.size(); i++) {
                            ...
                            for (int j = 0; j < elementList.size(); j++) {
                                ...
                                String elementName = element.getName();
                                String elementValue = element.attributeValue("value");
                                Object rsObject = ParameterUtil.getValue(busiConfig, paramName, elementValue, inParams, mapObject);
                                resultMap.put(elementName, rsObject);
                            }
                            result.add(resultMap);
                        }
                 ...
                 */
                    ...
                    if (ExpressionUtil.machExpression(patten2, value)) {
                        ...
                        List<Map<String, Object>> paramsArray = TypeUtil.changeToListMap(busiConfig.getParamsFromInParams_CmdResult(pn, inParams));
...
//                List<Map<String, Object>> paramsArray = busiConfig.getParamsFromInParams_CmdResult(paramName, inParams);
//                if (paramsArray.size() != 1)
